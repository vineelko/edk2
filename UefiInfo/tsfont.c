...
typedef struct _UEFIINFO_FONT_NODE {
    EFI_FONT_DISPLAY_INFO* Font;
    struct _UEFIINFO_FONT_NODE* Next;
} UEFIINFO_FONT_NODE, *PUEFIINFO_FONT_NODE;

static EFI_STATUS FontFreeFonts(PUEFIINFO_FONT_NODE FontList);

static EFI_STATUS FontEnumerateInstalledFonts(PUEFIINFO_FONT_NODE* FontList);
...
static EFI_STATUS FontEnumerateInstalledFonts(PUEFIINFO_FONT_NODE* FontList)
{
    ...
    PUEFIINFO_FONT_NODE RetFontList = NULL;
    ...
    RetFontList = AllocateZeroPool(sizeof(UEFIINFO_FONT_NODE));
    ...
    PUEFIINFO_FONT_NODE PrevNode = RetFontList;
    ...
    while (TRUE) {
        ...
        PUEFIINFO_FONT_NODE Node = AllocateZeroPool(sizeof(UEFIINFO_FONT_NODE));
...
static EFI_STATUS FontFreeFonts(PUEFIINFO_FONT_NODE FontList)
{
    PUEFIINFO_FONT_NODE Node = FontList;

    while (Node) {
        PUEFIINFO_FONT_NODE FreeNode = Node;
        Node = Node->Next;
...
static EFI_STATUS FontRasterString(IN EFI_FONT_DISPLAY_INFO* Font,
                                   IN EFI_HII_FONT_PROTOCOL* FontProtocol,
                                   IN EFI_GRAPHICS_OUTPUT_PROTOCOL* GraphicsProtocol,
                                   IN CHAR16* String,
                                   IN UINTN X,
                                   IN UINTN Y)
{
...
static EFI_STATUS FontRasterCharacter(IN EFI_FONT_DISPLAY_INFO* Font,
                                      IN EFI_HII_FONT_PROTOCOL* FontProtocol,
                                      IN EFI_GRAPHICS_OUTPUT_PROTOCOL* GraphicsProtocol,
                                      IN CHAR16 Char,
                                      IN UINTN X,
...
static EFI_STATUS FontListFontsProbe(IN PUEFIINFO_SESSION Session)
{
    EFI_STATUS Status = EFI_SUCCESS;
    PUEFIINFO_FONT_NODE FontList = NULL;
    UINTN i = 0;
    ...
    for (PUEFIINFO_FONT_NODE Node = FontList; Node; Node = Node->Next, i++) {
        DBG_INFO("Font %d", i);
...
static EFI_STATUS FontRasterStringProbe(IN PUEFIINFO_SESSION Session)
{
    EFI_STATUS Status = EFI_SUCCESS;
    EFI_HII_FONT_PROTOCOL* FontProtocol = NULL;
    ...
    UINTN X = 1;
    UINTN Y = 200;
    for (PUEFIINFO_FONT_NODE Node = FontList; Node; Node = Node->Next, Y += 80) {
...
static EFI_STATUS FontRasterCharacterProbe(IN PUEFIINFO_SESSION Session)
{
    EFI_STATUS Status = EFI_SUCCESS;
    EFI_HII_FONT_PROTOCOL* FontProtocol = NULL;
    EFI_GRAPHICS_OUTPUT_PROTOCOL* GraphicsProtocol = NULL;
...
FontListFontPackageProbe (
    IN PPROTOCOL_INFO ProtocolArray, IN PUEFIINFO_SESSION Session
    )
{
    EFI_STATUS Status = EFI_SUCCESS;
...
UEFIINFO_TEST_SUITE FontTestSuite = {
