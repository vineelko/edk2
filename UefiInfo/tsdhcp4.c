...
static VOID DumpDhcpPacket(EFI_DHCP4_PACKET* Packet)
{
    ...
    DBG_INFO("ServerName: %a", Packet->Dhcp4.Header.ServerName);
    DBG_INFO("BootFileName: %a", Packet->Dhcp4.Header.BootFileName);
}

static EFI_STATUS Dhcp4Probe(IN PUEFIINFO_SESSION Session)
{
    ...
    if (Mode.State == Dhcp4Init) {
        ...
        DBG_ERROR("Dhcp is not in expected state %a", Dhcp4StateMap[Mode.State].String);
        goto Exit;
    }
    ...
    DBG_INFO("DHCP 4 State: %a", Dhcp4StateMap[Mode.State].String);
...
UEFIINFO_TEST_SUITE Dhcp4TestSuite = {
