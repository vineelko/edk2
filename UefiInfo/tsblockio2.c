...
static EFI_STATUS UefiInfoBlockIo2(IN EFI_BLOCK_IO_PROTOCOL* BlockIoIf,
                                   IN EFI_BLOCK_IO2_PROTOCOL* BlockIo2If)
{
    ...
    if (IsRunningInVM() == TRUE) {
        //
        // Hyper-V and real hardware behave really different, particularly with
        // BlockIO, is that a the events are scheduled via a timer event on the
        // real hardware. Hyper-V has real hardware interrupt for I/O
        // operations, and the signal to for the completion event doesn't wait
        // for a timer tick interval. Blocking I/O on the real hardware is a cpu
        // spin until I/O complete.
        //
        ...
        // ready events. Eventually BlockIo2AsyncOperationCallback is called. It
        // will only be called only once (ie it doesn't count the signal events)
        // regardless of the number of completion event signals. This is why
        // each BlockIo2 has to use a different event.
        //
        // Due to this, on Hyper-V platform, usually Sync read/write are faster
        // compare to async read/write of the test results
...
static EFI_STATUS BlockIo2SyncAsync128KReadWriteTest(IN PUEFIINFO_SESSION Session)
{
...
extern EFI_STATUS BlockIo2WriteToAllSectors(IN PUEFIINFO_SESSION Session);
...
UEFIINFO_TEST_SUITE BlockIo2TestSuite = {
