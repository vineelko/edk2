...
static EFI_STATUS FileCreateDevicePathFromString(IN CHAR16* FileName,
                                                 OUT EFI_DEVICE_PATH_PROTOCOL** DevicePath);
static EFI_STATUS FileGetDevicePathInternal(IN EFI_HANDLE PartitionHandle,
                                            IN CHAR16* FileName,
                                            OUT EFI_DEVICE_PATH_PROTOCOL** DevicePathProtocol);

static CHAR16* EFIAPI WcstokCompat(IN CHAR16* String, IN const CHAR16* Delim, OUT CHAR16** SavePtr);
static UINTN EFIAPI WcsspnCompat(IN CHAR16* String, IN const CHAR16* String2);
static CHAR16* EFIAPI WcspbrkCompat(IN CHAR16* String, IN const CHAR16* String2);
...
EFI_STATUS EFIAPI FileDoesPartitionExist(IN CHAR16* PartitionName, OUT BOOLEAN* Exist)
{
...
EFI_STATUS EFIAPI FileGetSizeEx(IN CHAR16* PartitionName, IN CHAR16* FileName, OUT UINT64* FileSize)
{
    EFI_STATUS Status = EFI_SUCCESS;
    EFI_FILE_PROTOCOL* EfiFileProtocol = NULL;
                                    ...
                                    IN BOOLEAN RemovableOnly)
{
    ...
    if (Volumes == NULL) {
        ...
        CopyMem(Volumes, BufferGetContent(SimpleFileSystemBuffer), BufferSize);
                                           ...
                                           OUT EFI_FILE_PROTOCOL** EfiFileProtocol)
{
    ...
    CopyMem(DirectoryPathLocal, DirectoryPath, DirectoryPathSize);
                                                  ...
                                                  OUT EFI_FILE_PROTOCOL** EfiFileProtocol)
{
    ...
    CopyMem(FileNameLocal, FileName, FileNameSize);
...
static EFI_STATUS EFIAPI FileIsDirectory(IN EFI_FILE_PROTOCOL* DirFs)
{
    EFI_STATUS Status = EFI_SUCCESS;
...
//
EFI_STATUS EFIAPI FileCreateDirectory(IN EFI_FILE_PROTOCOL* DestDirectory, IN CHAR16* DirectoryName)
{
    EFI_STATUS Status = EFI_SUCCESS;
    EFI_FILE_PROTOCOL* NewDirectory = NULL;
...
static EFI_STATUS FileCopyToFile(IN EFI_FILE_PROTOCOL* SourceFile, IN EFI_FILE_PROTOCOL* DestFile)
{
    EFI_STATUS Status = EFI_SUCCESS;
    VOID* Buffer = NULL;
    UINTN BytesRead = 0;
...
//
static EFI_STATUS FileCopyToDirectory(IN EFI_FILE_PROTOCOL* SourceFile,
                                      IN EFI_FILE_PROTOCOL* DestDirectory)
{
    EFI_STATUS Status = EFI_SUCCESS;
...
//
static EFI_STATUS FileCopyRecursive(IN EFI_FILE_PROTOCOL* SourceDir, IN EFI_FILE_PROTOCOL* DestDir)
{
    EFI_STATUS Status = EFI_SUCCESS;
    EFI_FILE_INFO* FileInfo = NULL;
    EFI_FILE_PROTOCOL* File = NULL;
